...
class TestCLI(unittest.TestCase):
    ...
    def test_manage_remove(self):
        ...
        result = self.runner.invoke(app, ["manage", "remove", str(id_), "--delete"])
        self.assertEqual(result.exit_code,0)
        result = self.db.select_note_by_id(id_)
    ...
    def test_search_tag(self):
        ...
        note = Note(content="This is a hidden note!".encode("utf-8"), active=False)
        self.db.insert_note(note)
        id_ = self.db.last_row_id
        self.db.insert_tag(id_=id_, tags=["test"])
        ...
        result_id_list = self.get_all_ids_from_console(result.stdout)
        test_tag_id_list = [tag.fk_note_id for tag in self.db.select_tag("test")]
        ...
        result = self.runner.invoke(app, ["search", "tag", "-t", "test", "-t", "dev"])
        result_id_list = self.get_all_ids_from_console(result.stdout)
        dev_test_tag_id_list = [note.id_ for note in self.db.select_note_by_tags(["test", "dev"])]
